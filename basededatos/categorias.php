		...
		*/
		...
		case "update":
			...
			if (count($errors) == 0) {
				...
				$query = "UPDATE categorias set nombre = '$nombre', icon='$icon', id_padre = $value where id=$id";
		...
		case "insert":
			...
			if (count($errors) == 0) {
				...
				$query = "INSERT INTO categorias (nombre, icon, id_padre)  VALUES('$nombre','$icon', $value)";
								array_push($debug, "Query: " . $query);
				if (mysqli_query($db, $query)){
					/*
	...
	*/
	if (($view_mode == "edit") || ($view_mode == "add_new")){
		...
		<form action="categorias.php" method="post">
			...
			<?php if (!empty($icon))echo "<i class='$icon' aria-hidden='true'></i>"; ?>
			<div class="form-group">
